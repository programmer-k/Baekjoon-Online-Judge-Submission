...
void Solve() {
  ...
  candidate5 = quotient > 0 ? t * (quotient + 1) : numeric_limits<double>::infinity();

  cout << fixed << setprecision(10) << min({candidate1, candidate2, candidate3, candidate4, candidate5}) << '\n';
}
